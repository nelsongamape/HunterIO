...
public class stepsDefinition_APILeads extends DSL {
	...
	public void a_estrutura_de_response_body_esteja_conforme_a_documentacao() {
		...
		try {
			...
			Assert.assertTrue("Campo company_industry nao esta presente no body",
					companyIndustry.toString().matches("^.*|\\B$"));

			Object companySize = nodeData.get("company_size");
			Assert.assertTrue("Campo company_size nao esta presente no body",
					companySize.toString().matches("^.*|\\B$"));

			Object confidence_score = Integer.toString(nodeData.getInt("confidence_score"));
			Assert.assertTrue("Campo confidence_score nao esta presente no body",
					confidence_score.toString().matches("^\\d+$"));
			...
			Assert.assertTrue("Campo country_code nao esta presente no body",
					countryCode.toString().matches("^\\S+|\\B$"));
			...
			Assert.assertTrue("Campo linkedin_url nao esta presente no body",
					linkedinUrl.toString().matches("^\\S+|\\B$"));

			Object phoneNumber = nodeData.get("phone_number");
			Assert.assertTrue("Campo phone_number nao esta presente no body",
					phoneNumber.toString().matches("^\\S+|\\B$"));
			...
			Assert.assertTrue("Campo sending_status nao esta presente no body",
					sendingStatus.toString().matches("^\\S+|\\B$"));

			Object lastActivityAt = nodeData.get("last_activity_at");
			Assert.assertTrue("Campo last_activity_at nao esta presente no body",
					lastActivityAt.toString().matches("^\\S+|\\B$"));

			Object lastContactedAt = nodeData.get("last_contacted_at");
			Assert.assertTrue("Campo last_contacted_at nao esta presente no body",
					lastContactedAt.toString().matches("^\\S+|\\B$"));

			Object verification = nodeData.get("verification");
			Assert.assertTrue("Campo verification nao esta presente no body",
					verification.toString().matches("^\\S+|\\B$"));
	...
	public void que_sejam_enviados_apenas_os_campos_obrigatorios_do_request_body_preenchidos_e_preenchido_o_campo_confidence_score_com_valor_igual_a(
			Integer int1) {
	...
	public void que_sejam_enviados_apenas_os_campos_obrigatorios_do_request_body_preenchidos_e_o_campo_position_com_valor_igual_a(
			String position) {
